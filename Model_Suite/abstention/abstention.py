...
class AbstainingClassifier():
    ...
    def __init__(self, kw_args: dict, device: torch.device, class_weights=None, clc: bool = False):
        ...
        if self.ntask:
            ...
            self.ntask_acc_array = None  # this becomre a numpy array
        else:
        ...
        self.alphas = {task: float(kw_args['abstain_kwargs']["alphas"][task]) for
                       task in self.tasks}
        ...
        self.alpha_min_scale = {task: min(self.alpha_scale[task], 1.0 / self.alpha_scale[task])
                                for task in self.tasks}
        self.alpha_max_scale = {task: max(self.alpha_scale[task], 1.0 / self.alpha_scale[task])
                                for task in self.tasks}
        ...
        if clc:
            ...
            if self.multilabel:
                self.base_loss = torch.nn.BCEWithLogitsLoss(self.class_weights_tensor,
                                                            reduction='none')
            else:
                self.base_loss = torch.nn.NLLLoss(self.class_weights_tensor,
                                                  reduction='none')
    ...
    def compute_ntask_accuracy(self, y_true: list, y_pred: list):
        ...
        tmp = self.tasks[0]
        self.ntask_acc_array = np.ones(len(y_true[tmp]))[self.ntask_filter]
        for task in self.ntask_tasks:
            ...
            if _true.shape[0] > 0 and _pred.shape[0] > 0:
                ...
                self.ntask_acc = self.ntask_acc_array[self.ntask_acc_array == True].shape[0] / self.ntask_acc_array.shape[0]
            else:
                self.ntask_acc = 0.0
    ...
    def modify_ntask_alpha(self, additive=True):
        ...
        if self.ntask_acc == 0:
            acc_error = 1 - self.ntask_min_acc
