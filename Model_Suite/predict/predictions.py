...
class ScoreModel():
    ...
    """
    def __init__(self, model_args, data_loaders, dw, model, device, savepath=None, clc_flag=False):
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                if self.clc:
                    ...
                    y = {task: batch[f"y_{task}"].to(self.device)
                         for task in self.tasks if task != "Ntask"}
                    self.get_ys(logits, y, idxs)
                else:
                    y = {task: batch[f"y_{task}"].cpu().numpy()
                         for task in self.tasks if task != "Ntask"}
                ...
                # sys.stdout.write(f"sample {(i+1)*data_loader.batch_size} " +
    ...
    def compute_scores(self, dac=None):
        ...
        for task in self.tasks:
            ...
            if self.abstain:
                _trues_cpu = torch.tensor([v.item() for v in self.y_trues[task]], dtype=torch.int)[pred_idxs[task]]
                _preds_cpu = torch.tensor([v.item() for v in self.y_preds[task]], dtype=torch.int)[pred_idxs[task]]
                _trues = _trues_cpu.clone().to(self.device)
                _preds = _preds_cpu.clone().to(self.device)
            else:
                _trues = torch.tensor([v.item() for v in self.y_trues[task]], dtype=torch.int).to(self.device)
                _preds = torch.tensor([v.item() for v in self.y_preds[task]], dtype=torch.int).to(self.device)
    ...
    def predict(self,
        ...
        for split in data.keys():
            ...
            if save_probs:
                for task in self.tasks:
                    ...
                    probs_dfs.append(self.probs_to_dataframe(preds,
                                                             metadata[split],
                                                             id2label))
        ...
        if save_probs:
            ...
            probs_final.to_csv(
                self.savepath + f"_probs_{now}.csv",
                float_format='%7.6f',
                index=False)
    ...
    def evaluate_model(self, metadata: dict,
                       id2label: dict,
                       data_loaders=None,
                       save_probs=False,
                       dac=None,
            ...
            if save_probs:
                for task in self.tasks:
                    if task == "Ntask":
                        continue
                    preds[f"{task}_probs"] = preds['probs'][task]
                    probs_df.append(self.probs_to_dataframe(preds, metadata[split], id2label))
            ...
            for batch in data_loader:
                ...
                if self.clc:
                    ...
                    mask = torch.arange(
                        ...
                        device=self.device)[None, :] < batch_len[:, None]
    ...
    def get_ys(self, logits, y, idxs=None):
        ...
        for task in self.tasks:
            ...
            if self.clc:
                ...
                self.y_trues[task].extend(y[task][idxs].detach().cpu().numpy())
            else:
    ...
    def get_clc_preds(self, batch, preds, save_probs=False):
        ...
        for task in self.tasks:
            if task == "Ntask":
                ntask_prob = torch.sigmoid(logits['Ntask'][idxs])[:, -1]
