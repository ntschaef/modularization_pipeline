...
class ValidateParams():
    ...
    def check_data_files(self, data_path=None):
        ...
        if set(tasks).issubset(set(id2label.keys())) and len(tasks) > 0:
            pass
        else:
...
class ValidateClcParams():
    ...
    def __init__(self, cached_class: list, cli_args, data_source: str = 'pre-generated'):
        ...
        if len(cli_args.model_args) > 0:
            mod_args_file = cli_args.model_args + "/clc_args.yml"
        else:
    ...
    def check_data_train_args(self):
        ...
        schema = {'data_kwargs': ['tasks', 'exclude_single', 'shuffle_case_order',
                                  'split_by_tumorid', 'model_path',
                                  'random_seed', 'reproducible'],
                  'model_kwargs': ['att_dim_per_head', 'att_heads',
                                   'att_dropout', 'forward_mask'],
                  'train_kwargs': ['batch_per_gpu', 'max_epochs', 'patience', 'class_weights', 'mixed_precision']}
