...
def create_model(params, dw, device, embed_dim):
    ...
    """
    model = clc.CaseLevelContext(dw.num_classes, device=device, doc_embed_size=embed_dim,
...
def run_case_level(args):
    ...
    # batch = clc_params.model_args['train_kwargs']['batch_per_gpu']

    trainer = training.ModelTrainer(clc_params.model_args, model, dw,
                                    device=device, fold='clc',
    ...
    evaluator = predictions.ScoreModel(clc_params.model_args,
                                       dw.grouped_cases,
                                       dw,
                                       model,
                                       device,
    ...
    # oOnly need one of 5a, 5b, or 5c, depending on requirements
    # 5a. score a model
    ...
    if len(args.model_path) == 0:
        save_full_model(clc_params.model_args,
                        trainer.savename,
